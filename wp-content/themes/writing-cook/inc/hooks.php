 ...
 * Search by post name
 */ 
function post_search_ajax_handler() {
  $search_query_posts = sanitize_text_field($_POST['search_query_posts']);

  $args = array(
    'post_type' => 'post',
    'post_status' => 'publish',
    's' => $search_query_posts,
    'posts_per_page' => -1,
  );
  ...
  if ($query->have_posts()) { 
    ...
    <div class="container">
      <div class="searching__header">
        <h2 class="searching__title">Найдено постов: <?php echo $count ?></h2>
        <p class="searching__find extra-text">По запросу: "<?php echo $search_query_posts ?>"</p>
      </div>
      <div class="searching__list results-list">
        <?php
        while ($query->have_posts()) {
          ...
          get_template_part('template-parts/layouts/post-card');
  ...
  <?php } else { ?>
    <div class="container">
      <div class="searching__not-found">Увы, такие посты не найдены</div>
      <p class="searching__not-found-please">Попробуйте другой запрос</p>
    </div>
  <? }
...
add_action('wp_ajax_post_search', 'post_search_ajax_handler');
add_action('wp_ajax_nopriv_post_search', 'post_search_ajax_handler');
